...
export const Signup = async(req,res) =>{
  ...
  try {
    ...
    console.log(`Código de verificación para ${norm_email}: ${code}`);
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    const tokenData = JSON.stringify({email:norm_email});
...
export const validateResetPassword = async (req, res) => {
  ...
  try{
    ...
    console.error(error);
    res.status(500).json({ message: "Error al actualizar la contraseña, intentalo nuevamente o ve a la sección de ayuda" });
    resetPasswordCache.del(token);
  }
}